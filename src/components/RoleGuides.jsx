...
export default function RoleGuides() {
  ...
  const objections = [
    ...
    },
    {
      ...
      definition: "A witness may not testify to a matter unless evidence is introduced sufficient to support a finding that they have personal knowledge of the matter. Speculating on another person's thoughts or feelings is prohibited.",
    ...
    {
      rule: "Rule 702",
      name: "Improper Expert Opinion / Lack of Foundation",
    ...
    },
    {
      ...
      definition: "Although relevant, evidence may be excluded if its probative value is substantially outweighed by the danger of unfair prejudice, confusing the issues, or misleading the jury.",
  ...
  // Animate whole container when switching roles
  useEffect(() => {
  ...
  return (
    <div className="fade-in space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-800 pb-5">
        <div>
          <h1 className="text-xl font-serif font-bold text-white tracking-wide">COURTROOM ROLE PLAYBOOKS</h1>
          <p className="text-xs text-slate-400">Guides, checklists, and objections for every position on the Servite team.</p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {Object.keys(rolePlaybooks).map((role) => (
            <button
              ...
              className={`px-4 py-2 rounded text-xs font-serif font-bold uppercase tracking-wider transition-all border cursor-pointer ${activeRole === role ? 'bg-gold-brass/25 border-gold-primary text-gold-light' : 'border-slate-800 text-slate-400 hover:border-slate-700'}`}
            >
              {role}
            </button>
          ))}
      ...
      <div className="border-t border-slate-900 pt-8 space-y-6">
        <div>
          <h2 className="text-lg font-serif font-bold text-white tracking-wide">THE OBJECTION COACH</h2>
          <p className="text-xs text-slate-400">Quick reference courtroom objections under the CRF-OC Rules of Evidence.</p>
        </div>
